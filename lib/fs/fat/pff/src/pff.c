                ...
                )
{
    ...
    case FS_FAT12 : {
        ...
        ofs = bc % 512; bc /= 512;
        if (ofs != 511) {
            if (disk_readp(&fs->disk, buf, fs->fatbase + bc, ofs, 2)) break;
        } else {
            if (disk_readp(&fs->disk, buf, fs->fatbase + bc, 511, 1)) break;
            if (disk_readp(&fs->disk, buf+1, fs->fatbase + bc + 1, 0, 1)) break;
        }
        wc = LD_WORD(buf);
...
#if _FS_FAT16
    case FS_FAT16 :
        if (disk_readp(&fs->disk, buf, fs->fatbase + clst / 256, ((UINT)clst % 256) * 2, 2)) break;
        return LD_WORD(buf);
#endif
#if _FS_FAT32
    case FS_FAT32 :
        if (disk_readp(&fs->disk, buf, fs->fatbase + clst / 128, ((UINT)clst % 128) * 4, 4)) break;
        ...
        )
{
    ...
    do {
        res = disk_readp(&fs->disk, dir, dj->sect, (dj->index % 16) * 32, 32)	/* Read an entry */
                ? FR_DISK_ERR : FR_OK;
        if (res != FR_OK) break;
...
{
    if (disk_readp(&fs->disk, buf, sect, 510, 2))		/* Read the boot record */
        return 3;
    if (LD_WORD(buf) != 0xAA55)				/* Check record signature */
        return 2;

    if (!_FS_32ONLY && !disk_readp(&fs->disk, buf, sect, BS_FilSysType, 2) && LD_WORD(buf) == 0x4146)	/* Check FAT12/16 */
        return 0;
    if (_FS_FAT32 && !disk_readp(&fs->disk, buf, sect, BS_FilSysType32, 2) && LD_WORD(buf) == 0x4146)	/* Check FAT32 */
        return 0;
    return 1;
}
        ...
        )
{
    ...
    if (disk_initialize(&fs->disk) & STA_NOINIT)	/* Check if the drive is ready or not */
        return FR_NOT_READY;
    ...
    if (fmt == 1) {						/* Not an FAT boot record, it may be FDISK format */
        ...
        if (disk_readp(&fs->disk, buf, bsect, MBR_Table, 16)) {	/* 1st partition entry */
            fmt = 3;
        } else {
    ...
    if (disk_readp(&fs->disk, buf, bsect, 13, sizeof (buf))) return FR_DISK_ERR;
        ...
        )
{
    ...
    if (!fs) {				/* Check file system */
        ...
        dj->fn = sp;
        res = follow_path(fs, dj, dir, path);		/* Follow the path to the directory */
        if (res == FR_OK) {						/* Follow completed */
            if (dir[0]) {						/* It is not the root dir */
                if (dir[DIR_Attr] & AM_DIR)		/* The object is a directory */
                    dj->sclust = get_clust(fs, dir);
                else							/* The object is not a directory */
                    res = FR_NO_FILE;
            }
            if (res == FR_OK)
                res = dir_rewind(fs, dj);			/* Rewind dir */
        }
    }
        ...
        )
{
    ...
    if (!fs) {				/* Check file system */
        ...
        if (!fno) {
            ...
            res = dir_read(fs, dj, dir);	/* Get current directory item */
            if (res == FR_NO_FILE) res = FR_OK;
            if (res == FR_OK) {				/* A valid entry is found */
                get_fileinfo(dj, dir, fno);	/* Get the object information */
                res = dir_next(fs, dj);			/* Increment read index for next */
                if (res == FR_NO_FILE) res = FR_OK;
            }
        }
    }
